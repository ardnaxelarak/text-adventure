...
\chapter{Debug Mode}
Debug mode can be turned on with the command {\tt \bf debug}. This will allow the following commands, which should not generally used when playing the game, lest it spoil the fun.\footnote{Breaking the game while in debug mode is not covered by the warranty.}
\begin{itemize}
\item {\tt \bf acquire} will magically obtain an object no matter where it is, assuming the player does not have it, using the same syntax as {\tt \bf take}.
\item {\tt \bf poof} will teleport the player to a location of their choosing. Note that this requires knowing the name of the location in the code, which could be somewhat unintuitive for some locations.
\item {\tt \bf counters} will list all counters with their respective values
\item {\tt \bf showpeople} will list all people and show their current locations.
\end{itemize}
...
\chapter{Standalone Files}
\section{Getting Keypresses (keypress.pl)}
...
\section{People Wandering (peopleCode.pl)}
...
\section{Identifying Objects (identificationCode.pl)}
...
\section{Getting Commands from the User (getcommand.pl)}
...
\section{Forcing Output to Wrap at Words (writewrap.pl)}
